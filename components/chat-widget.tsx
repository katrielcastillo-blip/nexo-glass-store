    ...
    <div className="fixed bottom-4 right-4 z-40 sm:bottom-6 sm:right-6">
      {open && (
        <section className="glass mb-3 flex h-[min(570px,calc(100vh-7rem))] w-[calc(100vw-2rem)] max-w-[390px] flex-col overflow-hidden rounded-[2rem]" aria-label="Asistente de compras">
          ...
          {!apiKey ? (
            <div className="grid flex-1 place-items-center p-7 text-center"><div><span className="mx-auto mb-4 grid h-14 w-14 place-items-center rounded-2xl border border-amber-300/20 bg-amber-300/10 text-amber-300"><KeyRound /></span><h3 className="font-display text-xl font-bold">Configura tu API key de Gemini</h3><p className="mt-2 text-sm leading-relaxed text-slate-400">Añade tu clave de Google AI Studio para conversar con el asistente.</p><button className="primary-button mt-5" onClick={onSettings}>Abrir configuración</button></div></div>
          ) : (
