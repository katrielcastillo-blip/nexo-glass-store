...
export function SettingsModal({ open, apiKey, onClose, onSave }: SettingsModalProps) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Configuración" eyebrow="Asistente Gemini">
      <form onSubmit={submit}>
        ...
        <label className="block"><span className="mb-2 block text-sm font-bold">API key de Gemini</span><div className="relative"><input className="field pr-12" type={visible ? "text" : "password"} autoComplete="off" spellCheck={false} value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="AIza..." /><button type="button" className="focus-ring absolute right-2 top-1/2 grid h-9 w-9 -translate-y-1/2 place-items-center rounded-full text-slate-400 hover:bg-white/10 hover:text-white" onClick={() => setVisible((value) => !value)} aria-label={visible ? "Ocultar API key" : "Mostrar API key"}>{visible ? <EyeOff size={17} /> : <Eye size={17} />}</button></div></label>
        {draft.trim() && !isGoogleAiStudioKey && <p role="alert" className="mt-3 rounded-2xl border border-amber-300/25 bg-amber-300/10 px-4 py-3 text-sm leading-relaxed text-amber-100">Esta no parece una key de Google AI Studio. Pega una key de Gemini que empieza por <code>AIza…</code>.</p>}
